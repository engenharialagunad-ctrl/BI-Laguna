...
function getReportData() {
  return getReportDataFromSpreadsheet_(SpreadsheetApp.getActiveSpreadsheet());
}

function getReportDataFromSpreadsheet_(spreadsheet) {
  ...
  function isDataSheet(sheetName) {
    var normalizedName = normalizeText(sheetName);
    return normalizedName.indexOf("crt per") !== -1 || normalizedName.indexOf("usi per") !== -1;
  }
...
function buildExtractionDiagnosticMessage(diagnostic) {
  ...
  message.push("Abas candidatas com CRT PER/USI PER: " + (diagnostic.candidateSheets.length ? diagnostic.candidateSheets.join(", ") : "nenhuma"));
